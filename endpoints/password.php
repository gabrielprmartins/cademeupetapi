...
function api_password_lost($request) {
  ...
  if (empty($login)) {
    $response = new WP_Error('error', 'Informe o e-mail.', ['status' => 406]);
  ...
  $url = esc_url_raw($url . "/?key=$key&login=" . rawurldecode($user_email) . "\r\n");
  $body $message . $url;

  wp_mail($user_email, 'Reset de senha', $body);

  return rest_ensure_response('E-mail enviado com sucesso.');
}
